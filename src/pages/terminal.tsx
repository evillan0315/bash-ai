...
export default function Terminal() {
  ...
  const updateContextMenu = (input: string) => {
    const filtered = availableOptions.filter((opt) =>
      opt.toLowerCase().includes(input.toLowerCase().slice(1)) // remove leading slash
    );
  ...
  console.log(type, 'addEntry type');
  ...
  if (type === 'outputMessage'|| type==='error'&&content==='Authentication required') {
    ...
    if (outputDiv) {
      outputDiv.innerHTML = `<pre class="${type==='error'?'text-red-500':'text-yellow-500'} font-light whitespace-pre-wrap">${content}</pre>`;
    }
  ...
  onMount(() => {
    ...
    // 👉 Fetch token from localStorage or cookie (adjust if needed)
  ...
  s.on('connect', (e) => {
  console.log(s, 's');
    console.log(s.connected, '✅ Connected to WebSocket Gateway');
    if(!s.connected){
        setStatus('Connecting')
    	//addEntry('message', `${status()}`);
    }
    //setIsAuth(true)
    if(s.auth.token && s.connected){
        setIsAuth(true)
        setStatus('Connected')
        inputRef.disabled = false;
    	//addEntry('connect', `${status()}`);
    }
  ...
  return (
    <div class="flex flex-col h-screen bg-black text-white">
      <div class="flex items-center justify-between px-4 py-2 bg-zinc-900 border-b border-gray-900">
        <div class={`flex gap-2 ${status()==='Connected'?'text-green-400':'text-red-400'} items-center`}>
        {status()==='Connected'?<svg xmlns="http://www.w3.org/2000/svg" width="2em" height="2em" viewBox="0 0 24 24"><path fill="#00f02f" d="M21 11V9h-2V7a2.006 2.006 0 0 0-2-2h-2V3h-2v2h-2V3H9v2H7a2.006 2.006 0 0 0-2 2v2H3v2h2v2H3v2h2v2a2.006 2.006 0 0 0 2 2h2v2h2v-2h2v2h2v-2h2a2.006 2.006 0 0 0 2-2v-2h2v-2h-2v-2Zm-4 6H7V7h10Z"/><path fill="#00f02f" d="M11.361 8h-1.345l-2.01 8h1.027l.464-1.875h2.316L12.265 16h1.062Zm-1.729 5.324L10.65 8.95h.046l.983 4.374ZM14.244 8h1v8h-1z"/></svg>:(<svg xmlns="http://www.w3.org/2000/svg" width="2em" height="2em" viewBox="0 0 24 24"><path fill="#f00" d="M21 11V9h-2V7a2.006 2.006 0 0 0-2-2h-2V3h-2v2h-2V3H9v2H7a2.006 2.006 0 0 0-2 2v2H3v2h2v2H3v2h2v2a2.006 2.006 0 0 0 2 2h2v2h2v-2h2v2h2v-2h2a2.006 2.006 0 0 0 2-2v-2h2v-2h-2v-2Zm-4 6H7V7h10Z"/><path fill="#f00" d="M11.361 8h-1.345l-2.01 8h1.027l.464-1.875h2.316L12.265 16h1.062Zm-1.729 5.324L10.65 8.95h.046l.983 4.374ZM14.244 8h1v8h-1z"/></svg>)}
        
        <span class={`${status()==='Connected'?'text-green-500':'text-[#00f02f]'}`}>bashAI</span>
        ...
        <button
          ...
          title="Clear"
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="#fff" d="m14.03 1.889l9.657 9.657l-8.345 8.345l-.27.27H20v2H6.747l-3.666-3.666a4 4 0 0 1 0-5.657zm-8.242 11.07l-1.293 1.294a2 2 0 0 0 0 2.828l3.08 3.08h4.68l.366-.368z"/></svg>
        </button>
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 32 32"><circle cx="16" cy="8" r="2" fill="#fff"/><circle cx="16" cy="16" r="2" fill="#fff"/><circle cx="16" cy="24" r="2" fill="#fff"/></svg>
        </div>
      </div>

      <div
        class="flex-1 overflow-auto px-4 py-2 scroll-smooth text-sm "
        ref={outputRef}
        style={{ scrollBehavior: 'smooth' }} // Apply smooth scroll behavior
      >
        <div id="outputMessage" class="px-4 py-2 my-2 ">
        <pre
        ...
        <For each={entries()}>
          {(entry) => (
            <pre
              class={
                entry.type === 'command'
                  ? 'text-yellow-400 font-bold whitespace-pre-wrap'
                  : entry.type === 'error'
      ...
      {showContextMenu() && filteredOptions().length > 0 && (
        <div
          class="text-sm  bg-gray-900 text-white rounded-md shadow-lg z-10 border border-gray-600"
          style={{
        ...
        >
          <div class="flex flex-col text-left">
            <For each={filteredOptions()}>
              {(option) => (
                <button
                  class="text-gray-100 py-2 px-4 hover:bg-gray-800 text-left flex gap-2 items-center text-left"
  ...
  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
    <path fill="none" stroke="#fff" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M19 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V5a2 2 0 0 0-2-2M7 7h10M7 12h10M7 17h6"/>
      ...
      {/* 👇 Input Area */}
      <div class="flex items-center justify-between pb-4 gap-2 bg-zinc-800 relative">
        <span class={`ml-4 ${status()==='Connected'?'text-green-400':'text-red-400'}`}>$</span>
        <input
          ref={inputRef}
          class={`flex-1 bg-black ${status()==='Connected'?'text-green-400':'text-red-400'} px-1 text-sm rounded-md`}
          type="text"
